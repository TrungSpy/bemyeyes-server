...
var last_run = new Date(2012,1,1);
db.event_logs.find({name:'request_created',created_at:{$gte:last_run}}).sort({created_at:1}).forEach(function(event_log) {
  var call_length = 0;
  var wait_length = 0;
  ...
  if (answered && stopped) {
    call_length = Math.abs(stopped.created_at - answered.created_at)/1000;
    wait_length = Math.abs(request_created - answered.created_at)/1000;
  }
